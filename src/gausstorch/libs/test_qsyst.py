"""
Test module for the :py:mod:`gausstorch.libs.qsyst` module.
"""
...
def test_alpha_after_time_evolution_g_only() -> None:
    ...
    theta_encoded = model.encode_theta(
        theta_0=theta_0,
        x_mask=x_mask,
        x_min=x_min,
        x_max=x_max,
    ...
    alpha_1_theory = (
        torch.sqrt(pars["k_ext"][0])
        * (-alpha_in_scalar)
        * (
            (
                torch.exp(
                    (-1j * pars["g_real"][0] - (pars["k_ext"][0] / 2)) * pars["t_i"]
                )
                - 1
            )
