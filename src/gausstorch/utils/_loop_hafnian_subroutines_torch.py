...
def precompute_binoms(max_binom):
    ...
    """
    binoms = torch.zeros((max_binom + 1, max_binom + 1), dtype=torch.int64)
...
def matched_reps(reps):
    ...
    x = torch.tensor(edgesA + edgesB, dtype=torch.int64)  # reordered list of indices
    edgereps = torch.tensor(edgereps, dtype=torch.int64)
...
def find_kept_edges(j, reps):
    ...
    output = []
    bases = reps.clone() + 1
    for base in bases.flip(0):
        output.append(num % base)
        num //= base
    return torch.tensor(output[::-1], dtype=reps.dtype)
...
def f(E, n):
    ...
    count = 0
    comb = torch.zeros((2, n // 2 + 1), dtype=torch.complex128)
    comb[0, 0] = 1
    for i in range(1, n // 2 + 1):
        ...
        powfactor = 1
        count = 1 - count
        comb[count, :] = comb.clone()[1 - count, :]  # remove in place operations
...
def f_loop(E, AX_S, XD_S, D_S, n):
    ...
    for i in range(1, n // 2 + 1):
        ...
        E_k = E_k.clone()
        # E_k *= E
        XD_S = XD_S.clone() @ AX_S
        powfactor = torch.tensor(1.0, dtype=torch.float64)  # modified 17/05/2024
        count = 1 - count
        comb[count, :] = comb.clone()[1 - count, :]
        for j in range(1, n // (2 * i) + 1):
...
def f_loop_odd(E, AX_S, XD_S, D_S, n, oddloop, oddVX_S):
    ...
    count = 0
    comb = torch.zeros((2, n + 1), dtype=torch.complex128)
    comb[0, 0] = 1
...
def get_AX_S(kept_edges, A):
    ...
    z = torch.cat((kept_edges, kept_edges))
    nonzero_rows = torch.nonzero(z != 0).flatten()
...
def get_submatrices(kept_edges, A, D, oddV):
    ...
    XD_nonzero = torch.empty_like(D_nonzero, dtype=torch.complex128)
...
def calc_approx_steps(fixed_reps, N_cutoff):
    steps = int(torch.prod(torch.sqrt(fixed_reps)) + 1) * N_cutoff // 2
    return steps
