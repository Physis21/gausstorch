...
g_shape = (
    M * (M - 1) // 2
)  # number of possible couplings for a given coupling type (photon conversion or two-mode squeezing)
eA = M * 1e5
init_pars = {
    "M": M,  # Number of modes
...
alpha, sigma = model.alpha_sigma_evolution(
    model.other_pars["t_i"], model.alpha0, model.sigma0, "eA", theta_encoded
)
